...
def nfw_test(halo_cutout_dir='/Users/joe/repos/mpwl-raytrace/NFW_test_cases/lensing_output', 
    ...
    pprint('Finding source plane runs')
    zspl_dirs = glob.glob('{}/lensmaps*'.format(halo_cutout_dir))
    zspl = sorted([float(d.split('zs')[-1]) for d in zspl_dirs])

    for i in range(len(zspl)):
        pprint('reading lensing mock data for source plane {}'.format(zspl[i]))
        [sim_lens, true_profile] = read_nfw(halo_cutout_dir, zspl[i])
        out_dir = '{}/profile_fits'.format(halo_cutout_dir)
        if not os.path.exists(out_dir): os.makedirs(out_dir)
        fit_nfw(sim_lens, true_profile, zspl[i], showfig=showfig, 
                out_dir=out_dir, makeplot = makeplot, bin_data=bin_data, 
                rbins=rbins, rmin=rmin, nfw=nfw)


def read_nfw(halo_cutout_dir, zspl):

    # get ray-trace hdf5 and properties csv
    rtfs = glob.glob('{}/lensmaps_zs{}/*mock*.hdf5'.format(halo_cutout_dir, zspl))
...
def fit_nfw(lens, true_profile, zspl, makeplot=True, showfig=False, out_dir='.', 
    ...
    ax.set_title(r'$z_\mathrm{{source}} = {}$'.format(zspl), fontsize=14)
    ...
    if(bin_data and nfw):
        f.savefig('{}/{}_shearprof_fit_{}bins_{}rmin_nfw_zs{}.png'.format(out_dir, zl, rbins, rmin, zspl), dpi=300)
    if(bin_data and not nfw):
        f.savefig('{}/{}_shearprof_fit_{}bins_{}rmin_zs{}.png'.format(out_dir, zl, rbins, rmin, zspl), dpi=300)
    else:
        f.savefig('{}/{}_shearprof_fit_{}rmin_zs{}.png'.format(out_dir, zl, rmin, zspl), dpi=300)
